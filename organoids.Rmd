---
title: "First report: Kaftrio® vs. Symkevi® in intestinal organoids\n Supplementary Statistical Analysis"
abstract: "This document contains exploratory plots and formal statistical analysis for the data reported in the main manuscript. Complete source code and all data to reporduce this report are available via Zenodo, DOI: 10.5281/zenodo.4478985 "
output: 
...
```{r area-control, fig.cap = "The raw area values of the control condition for each Forskolin concentration over time. Each line represents a single well."}
raw_data %>% 
  filter(type == "FskOnly") %>%
...
```{r swell-control, fig.cap = "The swell values of the control condition for each Forskolin concentration over time. Each line represents a single well."}
raw_data %>% 
  filter(type == "FskOnly") %>%
...
The `diff` column represents actual expected difference in AUC between the two conditions,
...
`diff` column and the associated confidence intervals are on the log scale. First we list those differences where adjusted p < 0.001:
...
The mixed model for area from the previous section predicts some improvement for Kaftrio over Symkevi for all patients, but also has trouble predicting the values for the 58-CF patient well as witnessed by the posterior predictive check in Figure \@ref(fig:ppcheck58).
